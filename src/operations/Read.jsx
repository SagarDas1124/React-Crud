...
export const Read = () => {
  ...
  const getData=async()=>{
    try{
        const response= await axios.get("https://crud-express-bend.onrender.com/api/v1/employee")
  ...
  const handleDelete=async(id)=>{
    try{
        await axios.delete(`https://crud-express-bend.onrender.com/api/v1/employee/${id}`)
        getData();
    }catch(error){
      console.log(error.message);
    }
